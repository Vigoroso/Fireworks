...
void display(void)
{
	...
	char vel[30];
	BaseSystem* tankPath = GlobalResourceManager::use()->getSystem("tankpath");
	if (tankPath)
	{
		double carVel = ((Tank*)tankPath)->getVel();
		if ((carVel < 1000.0 ) && (carVel >= 0.0))
		{
			sprintf(vel, "Car speed: %.3lf %c", carVel, '\0');
			GLlabel(vel, 15);
		}
	}
