...
bool ResourceManager::removeObject(BaseObject* newObject, bool visible)
{
	ListOfObjects::const_iterator iter;
	for (iter = m_objectList.begin(); iter != m_objectList.end(); iter++)
	{
		if ((*iter)->retrieveName() == newObject->retrieveName())
		{
			m_objectList.erase(iter);
			m_objectDatabase.erase(newObject->retrieveName());
			//delete (*iter);
			return true;
		}
...
bool ResourceManager::removeSystem(BaseSystem* newSystem, bool visible)
{
	ListOfSystems::const_iterator iter;
	for (iter = m_systemList.begin(); iter != m_systemList.end(); iter++)
	{
		if ((*iter)->retrieveName() == newSystem->retrieveName())
		{
			m_systemList.erase(iter);
			m_objectDatabase.erase(newSystem->retrieveName());
			m_displaylist.erase(newSystem);
			//delete (*iter);
			return true;
		}
...
bool ResourceManager::removeSimulator(BaseSimulator* newSimulator, bool visible)
{
	ListOfSimulators::const_iterator iter;
	for (iter = m_simulatorList.begin(); iter != m_simulatorList.end(); iter++)
	{
		if ((*iter)->retrieveName() == newSimulator->retrieveName())
		{
			m_simulatorList.erase(iter);
			m_objectDatabase.erase(newSimulator->retrieveName());
			//delete (*iter);
			return true;
		}
...
void ResourceManager::stepAllSimulators()
{
	...
	for( iter = m_simulatorList.begin(); iter != m_simulatorList.end(); iter++ )
	{
		if ((*iter)->step(m_simulationTime) == 1)
		{
			break;
		}
	...
	// after performing steps, check to see if any sims need deleting
	for (iter = m_simulatorList.begin(); iter != m_simulatorList.end(); iter++)
	{
		if ((*iter)->MarkedForDelete)
		{
			ParticlePhysicsSimulator* ptr = dynamic_cast<ParticlePhysicsSimulator*>(*iter);
			removeSystem(ptr->m_ParticleSystem);
			removeSimulator((*iter));
			break;
		}
	}
